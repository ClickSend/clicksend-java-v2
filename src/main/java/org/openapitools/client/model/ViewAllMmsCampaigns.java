...
import org.openapitools.client.model.ViewAllMmsCampaignsData;
...
public class ViewAllMmsCampaigns {
   ...
   * @throws IOException if the JSON Element is invalid with respect to ViewAllMmsCampaigns
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ViewAllMmsCampaigns.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in ViewAllMmsCampaigns is not found in the empty JSON string", ViewAllMmsCampaigns.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ViewAllMmsCampaigns.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `ViewAllMmsCampaigns` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ViewAllMmsCampaigns.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ViewAllMmsCampaigns' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ViewAllMmsCampaigns> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ViewAllMmsCampaigns.class));

       return (TypeAdapter<T>) new TypeAdapter<ViewAllMmsCampaigns>() {
           @Override
           public void write(JsonWriter out, ViewAllMmsCampaigns value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ViewAllMmsCampaigns read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of ViewAllMmsCampaigns given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of ViewAllMmsCampaigns
   * @throws IOException if the JSON string is invalid with respect to ViewAllMmsCampaigns
   */
  public static ViewAllMmsCampaigns fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ViewAllMmsCampaigns.class);
  }

  /**
   * Convert an instance of ViewAllMmsCampaigns to an JSON string
   *
   * @return JSON string
   */
